...
st.set_page_config(layout="wide", page_title="Analsis de Informalidad Laboral", page_icon="icons/busqueda-de-trabajo.png")
...
NOMBRE_ARCHIVO_EXCEL = "datos/tasa de informalidad laboral de asalariados segun caracteristicas socioeconomicas.xlsx"

try:
    dfDatos = pd.read_excel(NOMBRE_ARCHIVO_EXCEL, header=3)
    dfDatos = dfDatos.dropna(how="all")
    dfDatos = dfDatos[dfDatos.iloc[:, 0].notna()]

    st.success(f"Archivo '{NOMBRE_ARCHIVO_EXCEL}' cargado correctamente.")
except FileNotFoundError:
    st.error(f"Archivo '{NOMBRE_ARCHIVO_EXCEL}' no encontrado.")
    st.stop()

# Captura del orden original
dfDatos["Orden_Datos"] = dfDatos.index


def separar_categoria_item(valor):
...
dfDatos[["Categoria", "Item"]] = dfDatos.iloc[:, 0].apply(
    lambda x: pd.Series(separar_categoria_item(str(x)))
)
...
id_vars = ["Categoria", "Item"]
id_vars_melt = id_vars + ["Orden_Datos"]

columnas_temporales = dfDatos.columns[1:-1].tolist()
...
dfDatosLargo["Valor_Numérico"] = pd.to_numeric(dfDatosLargo["Valor_Numérico"], errors="coerce")
...
# ==============================================================================
# TENDENCIA HISTÓRICA + VALOR RECIENTE
# ==============================================================================
...
dfValoresRecientes = dfDatosLargo[dfDatosLargo["Trimestre"] == trimestre_reciente].copy()

dfValoresRecientes = dfValoresRecientes.rename(columns={
    "Valor_Numérico": "Valor_Reciente",
    "Trimestre": "Trimestre_Reciente"
})

dfValoresRecientes = dfValoresRecientes[
    id_vars + ["Orden_Datos", "Valor_Reciente", "Trimestre_Reciente",
...
dfAnalisis = dfAnalisis[[
    nombre_col_categoria, "Valor_Reciente", "Promedio_Historico",
...
with st.expander("Informe de datos de Empleo- Tablas- avamzadas", expanded=False, icon=":material/work_alert:", width="stretch"):
    ...
    st.title("Reporte de Datos por Categoría y Tendencia Histórica :material/breaking_news:")

    parCategoria = st.multiselect(
        f"Selecciona la/las {nombre_col_categoria}(s)",
    ...
    with tabTabla:
        table = (
            ...
            .tab_header(
                ...
                subtitle=html(f"Valor más reciente: **{trimestre_reciente_display}**"),
            )
            .fmt_nanoplot("Valor_Numérico")
            .fmt_number(columns="Valor_Reciente", decimals=1)
            .fmt_percent(columns=["Cambio_Porcentual_Reciente", "Cambio_Porcentual_YoY"], decimals=1)
            .cols_label(
                Valor_Numérico="Tendencia Histórica",
                Valor_Reciente="Valor Reciente",
                Cambio_Porcentual_Reciente="Cambio % (vs T-1)",
                Cambio_Porcentual_YoY="Cambio % (YoY)",
                Trimestre_Reciente="Período Reciente"
            )
            .tab_style(style=style.text(color="red", weight="bold"),
                    locations=loc.body(columns="Cambio_Porcentual_Reciente",
                                        rows=pl.col("Cambio_Porcentual_Reciente") < 0))
            .tab_style(style=style.text(color="green", weight="bold"),
                    locations=loc.body(columns="Cambio_Porcentual_Reciente",
                                        rows=pl.col("Cambio_Porcentual_Reciente") > 0))
            .tab_style(style=style.text(color="red", weight="bold"),
                    locations=loc.body(columns="Cambio_Porcentual_YoY",
                                        rows=pl.col("Cambio_Porcentual_YoY") < 0))
            .tab_style(style=style.text(color="green", weight="bold"),
                    locations=loc.body(columns="Cambio_Porcentual_YoY",
                                        rows=pl.col("Cambio_Porcentual_YoY") > 0))
            .tab_style(style=[style.text(weight="bold"), style.fill(color="#D9E9CF")],
                    locations=loc.row_groups())
            .tab_style(style=style.text(weight="bold"),
                    locations=loc.body(columns=nombre_col_item))
            .tab_source_note("Fuente: Base EPH – Informalidad laboral.")
            .as_raw_html()
        )
        st.write(table, unsafe_allow_html=True)

    with tabDatos:
        st.subheader("Datos Fuente (Transformados)")
    ...
    st.markdown("---")
    st.header("🔍 Análisis Resumen por Categoría")

    table_analisis = (
        GT(pl.from_pandas(dfAnalisis), rowname_col=nombre_col_categoria)
        .fmt_number(columns=[f"Valor Reciente ({trimestre_reciente_display})", "Promedio Histórico"], decimals=1)
        .fmt_percent(columns=["Cambio_vs_PH", "Cambio % (YoY Promedio)"], decimals=1)
        .tab_style(style=style.text(color="red", weight="bold"),
...
with st.expander("Modelo Estadístico Avanzado", expanded=False, icon=":material/work_alert:", width="stretch"):
    ...
    # ----------------------------
    FILE_PATH = "datos/tasa de informalidad laboral de asalariados segun caracteristicas socioeconomicas.xlsx"

    st.header("Dashboard Estadístico Avanzado — Tasa de Subocupación")
...
with st.expander("Acceso a botones del Menú", expanded=False, icon=":material/auto_stories:", width="stretch"):
    ...
    with st.container(border=True):
            ...
            with col5:    
                if st.button("Desocupación", key="btn-liquid", use_container_width=True):
                    st.switch_page("pages/2_desocupacion.py")

            with col6:
                if st.button("Subocupación", key="btn-glitch", use_container_width=True):
